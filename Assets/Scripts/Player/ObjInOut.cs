...
public class ObjInOut : MonoBehaviour
{
    ...
    private static bool inMoveObj = false;
    public static bool InMoveObj
    {
        get { return inMoveObj; }
        set { inMoveObj = value; }
    }
    ...
    public void TakeInMoveObj()
    {
        if (toMoveObj != null && !inMoveObj)//箱があるかどうか
        {
            ...
            // 箱が大きなったら取り込めない
            if(toMoveObj.transform.localScale.x == 1 || toMoveObj.transform.localScale.x == 2)
            {
                if (rb2dCheck.mass == 1)
                {
