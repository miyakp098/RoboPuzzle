...
public class ObjMove2 : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        //if (transform.localScale.x == 1 && !detachChildObj)
        //{
        //    moveObjTrigger.isActive = true;//moveObjTriggerを有効にする
        //    moveObjTrigger2.isActive = false;
        //}
        //else if(transform.localScale.x == 2 && !detachChildObj)
        //{

        //    moveObjTrigger.isActive = false;//moveObjTriggerを無効にする
        //    moveObjTrigger2.isActive = true;
        //}
        //else
        //{
        ...
        ////箱の上にオブジェクトがあるかの判定
        //Vector2 moveObjOnPos = new Vector2(transform.position.x, transform.position.y + 0.8f * transform.localScale.y);
        //Vector2 moveObjOnArea = new Vector2(0.4f * transform.localScale.x, 0.1f * transform.localScale.y); //地面判定エリア
        //Debug.DrawLine(moveObjOnPos + moveObjOnArea, moveObjOnPos - moveObjOnArea, Color.red);
        //objectOnThisObj = Physics2D.OverlapArea(moveObjOnPos + moveObjOnArea, moveObjOnPos - moveObjOnArea);
        ...
        if (onObject && !moveObjTrigger.OnMoveObj && !moveObjTrigger2.OnMoveObj)
        {
            ...
            if (x > 0)//Player⇔Obj
            {
                offset = transform.localScale.x / 2 + 0.35f;//プレイヤーの大きさによって変える
            }
            ...
            //Vector2 touchPos = new Vector2(transform.position.x, transform.position.y);
            //Vector2 touchArea = new Vector2(0.55f, 0.4f);//壁判定エリア
            //Debug.DrawLine(touchPos + touchArea, touchPos - touchArea, Color.blue);
            //isWall = Physics2D.OverlapArea(touchPos + touchArea, touchPos - touchArea, objLayer);
